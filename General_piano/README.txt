...
My system requires inputs from the keyboard [pygame]

THE NOTE MUST CEASE AFTER USER LETS GO OFF THE KEY [pygame]

each stroke translates to a specific note on the keyboard

that note needs to work along with other notes, so polyphony [pygame]

so we need a bounded buffer that works within the audible signal frequencies [pygame]

there was an issue with moving useful notes out of the previous folder to the new one 
I used gemini to make some PowerShell commands since idk PowerShell and it actually moved 79 
.mp3 files of the piano, saying that high pitch piano notes sound metallic and are not generally used
I am not an expert in audio stuff so I will take what I can from that XD. 

NO AI ON PYTHON CODE THOUGH, PINKY PROMISE.
